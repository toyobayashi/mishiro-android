...
std::string Reader::packageName = PACKAGE_NAME;
...
Reader::Reader(unsigned char* buf, unsigned int length) {
  ...
  tmpfile = "/storage/emulated/0/Android/data/" + Reader::packageName + "/cache/" + tmpname.substr(tmpname.find_last_of('/') + 1);
  auto posl = tmpfile.find_last_of('/');
  ...
  if (posl == std::string::npos) {
    ...
    dirname = tmpfile.substr(0, posl);
    filename = tmpfile.substr(posl + 1);
  }
  ...
  std::ofstream fs(tmpfile, std::ios::binary | std::ios::out);
