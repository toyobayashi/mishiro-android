...
class Global {
  ...
  private _se: HTMLAudioElement = new Audio()
  private _seOk: HTMLAudioElement = new Audio()
  ...
  constructor (options: GlobalConstructorArgument) {
    ...
    if (options.se) this._se.src = options.se
    if (options.seOk) this._seOk.src = options.seOk
