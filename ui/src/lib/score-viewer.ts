...
import { writeFile } from '../native/cordova-fs'
...
class ScoreViewer {
  ...
  private _oncanplay = () => {
    this._isReady = true
    this.rangeInput.max = globalInstance.audio.duration.toString()
  }
  ...
  private _ontimeupdate = () => {
    this.rangeInput.value = globalInstance.audio.currentTime.toString()
    this.rangeInput.style.backgroundSize = 100 * (globalInstance.audio.currentTime / globalInstance.audio.duration) + '% 100%'
  }

  public removeAllEventListener () {
    globalInstance.audio.removeEventListener('canplay', this._oncanplay)
    globalInstance.audio.removeEventListener('play', this._onplay)
    globalInstance.audio.removeEventListener('pause', this._onpause)
    globalInstance.audio.removeEventListener('timeupdate', this._ontimeupdate)
  ...
  constructor (song: Song<ScoreNote>, el: HTMLElement, options?: Option) {
    ...
    globalInstance.audio.src = song.src
    globalInstance.audio.addEventListener('canplay', this._oncanplay)
    globalInstance.audio.addEventListener('play', this._onplay)
    globalInstance.audio.addEventListener('pause', this._onpause)
    globalInstance.audio.addEventListener('timeupdate', this._ontimeupdate)
  ...
  private _cal () {
    ...
    for (let i = 0; i < this.song.score.length; i++) {
      if (this.song.score[i].sec > globalInstance.audio.currentTime) {
        combo = i
        break
      }
      (this.song.score[i]._instance as Note).setY(ScoreViewer.calY(this.options.speed, this.song.score[i].sec, globalInstance.audio.currentTime))
    }
  ...
  private async _saveScore () {
    ...
    const _drawAndSave = async () => {
      if (!this._isReadyToSave) {
        this.stop()
        this.saveCanvas.height = globalInstance.saveSpeed * 60 * globalInstance.audio.duration / globalInstance.scale
      ...
      if (window.cordova) {
        const savePath = `${window.cordova.file.externalDataDirectory}score/${ScoreViewer._id}-${ScoreViewer._difficulty}.png`
        await writeFile(savePath, blob)
        await toast('Save success: ' + savePath, 'long')
        // ;(cordova.plugins as any).MyToast.toast(`Success.\n${window.cordova.file.externalDataDirectory}${ScoreViewer._id}-${ScoreViewer._difficulty}.png`, 'short', () => console.log('toast'), (e: Error) => console.log(e))
      } else {
  ...
  private _rangeOnInput = (ev: Event) => {
    globalInstance.audio.currentTime = Number((ev.target as HTMLInputElement).value)
  }

  private _speedOnInput = (ev: Event) => {
  ...
  private _resolveDOM (el: HTMLElement) {
    ...
    this.rangeInput.addEventListener('input', this._rangeOnInput, false)
    ...
    this.speedInput.addEventListener('input', this._speedOnInput, false)
    ...
    if (globalInstance.backPng && globalInstance.backPng.src) {
      ...
      const onload = function onload (this: HTMLImageElement) {
        ...
        liveIcon.removeEventListener('load', onload)
      }
      liveIcon.addEventListener('load', onload, false)
